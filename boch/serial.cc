...
void bx_serial_c::init(void)
{
    const Bit8u port = 0;
    const Bit16u base = 0x03f8;
    const Bit8u irq = 4;
    const char* name = "Serial Port 1";
    ...
    BX_SER_THIS s[port].IRQ = irq;
    DEV_register_irq(BX_SER_THIS s[port].IRQ, name);
    ...
    if (BX_SER_THIS s[port].tx_timer_index == BX_NULL_TIMER_HANDLE) {
        BX_SER_THIS s[port].tx_timer_index =
            DEV_register_timer(this, tx_timer_handler, 0, 0, 0, "serial.tx");
        bx_pc_system.setTimerParam(BX_SER_THIS s[port].tx_timer_index, port);
    }

    if (BX_SER_THIS s[port].rx_timer_index == BX_NULL_TIMER_HANDLE) {
        BX_SER_THIS s[port].rx_timer_index =
            DEV_register_timer(this, rx_timer_handler, 0, 0, 0, "serial.rx");
        bx_pc_system.setTimerParam(BX_SER_THIS s[port].rx_timer_index, port);
    }

    if (BX_SER_THIS s[port].fifo_timer_index == BX_NULL_TIMER_HANDLE) {
        BX_SER_THIS s[port].fifo_timer_index =
            DEV_register_timer(this, fifo_timer_handler, 0, 0, 0, "serial.fifo");
        bx_pc_system.setTimerParam(BX_SER_THIS s[port].fifo_timer_index, port);
    }

    BX_SER_THIS s[port].int_enable.rxdata_enable = 0;
    BX_SER_THIS s[port].int_enable.txhold_enable = 0;
    BX_SER_THIS s[port].int_enable.rxlstat_enable = 0;
    BX_SER_THIS s[port].int_enable.modstat_enable = 0;

    BX_SER_THIS s[port].int_ident.ipending = 1;
    BX_SER_THIS s[port].int_ident.int_ID = 0;

    BX_SER_THIS s[port].fifo_cntl.enable = 0;
    BX_SER_THIS s[port].fifo_cntl.rxtrigger = 0;
    BX_SER_THIS s[port].rx_fifo_end = 0;
    BX_SER_THIS s[port].tx_fifo_end = 0;

    BX_SER_THIS s[port].line_cntl.wordlen_sel = 0;
    BX_SER_THIS s[port].line_cntl.stopbits = 0;
    BX_SER_THIS s[port].line_cntl.parity_enable = 0;
    BX_SER_THIS s[port].line_cntl.evenparity_sel = 0;
    BX_SER_THIS s[port].line_cntl.stick_parity = 0;
    BX_SER_THIS s[port].line_cntl.break_cntl = 0;
    BX_SER_THIS s[port].line_cntl.dlab = 0;

    BX_SER_THIS s[port].modem_cntl.dtr = 0;
    BX_SER_THIS s[port].modem_cntl.rts = 0;
    BX_SER_THIS s[port].modem_cntl.out1 = 0;
    BX_SER_THIS s[port].modem_cntl.out2 = 0;
    BX_SER_THIS s[port].modem_cntl.local_loopback = 0;

    BX_SER_THIS s[port].line_status.rxdata_ready = 0;
    BX_SER_THIS s[port].line_status.overrun_error = 0;
    BX_SER_THIS s[port].line_status.parity_error = 0;
    ...
    BX_SER_THIS s[port].line_status.fifo_error = 0;
    ...
    BX_SER_THIS s[port].scratch = 0;
    BX_SER_THIS s[port].rxbuffer = 0;
    BX_SER_THIS s[port].thrbuffer = 0;
    BX_SER_THIS s[port].tsrbuffer = 0;

    BX_SER_THIS s[port].divisor_lsb = 1;
    BX_SER_THIS s[port].divisor_msb = 0;
    BX_SER_THIS s[port].baudrate = 115200;
    BX_SER_THIS s[port].databyte_usec = 87;

    BX_SER_THIS s[port].io_mode = BX_SER_MODE_NULL;
    BX_SER_THIS s[port].tty_id = -1;
    BX_SER_THIS s[port].socket_id = (SOCKET)-1;
    BX_SER_THIS s[port].output = NULL;
    BX_SER_THIS s[port].file = NULL;

#ifdef BX_SER_WIN32
    BX_SER_THIS s[port].pipe = INVALID_HANDLE_VALUE;
#endif

    for (unsigned addr = base; addr < (unsigned)(base + 8); addr++) {
        if (addr < (unsigned)(base + 7)) {
    ...
    //BX_INFO(("com1 at 0x%04x irq %d (mode: null)", base, irq));
}
...
void bx_serial_c::tx_timer(void)
{
    ...
    case BX_SER_MODE_TERM:
        ...
        if (BX_SER_THIS s[port].tty_id >= 0) {
            _write(BX_SER_THIS s[port].tty_id, (bx_ptr_t)&BX_SER_THIS s[port].tsrbuffer, 1);
        }
#endif
        break;
    case BX_SER_MODE_RAW:
    ...
    if ((BX_SER_THIS s[port].line_status.rxdata_ready == 0) ||
        ...
        case BX_SER_MODE_TERM:
            ...
            if ((BX_SER_THIS s[port].tty_id >= 0) && (select(BX_SER_THIS s[port].tty_id + 1, &fds, NULL, NULL, &tval) == 1)) {
                (void)_read(BX_SER_THIS s[port].tty_id, &chbuf, 1);
