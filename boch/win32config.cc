...
#ifndef BX_LOG_OPTS_EXCLUDE
#define BX_LOG_OPTS_EXCLUDE(type, choice) \
    (((type) <= 1 && (choice) >= 2) || ((type) == 3 && (choice) == 0))
#endif

const char log_choices[6][16] = { "ignore", "log", "warn user", "ask user", "end simulation", "no change" };
...
void InitLogOptionsDialog(HWND hDlg, BOOL advanced)
{
    ...
    idx = SendMessage(GetDlgItem(hDlg, IDDEVLIST), LB_ADDSTRING, 0, (LPARAM)"bochs");
    SendMessage(GetDlgItem(hDlg, IDDEVLIST), LB_SETITEMDATA, idx, 0);
    if (advanced) {
