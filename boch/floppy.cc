...
void bx_floppy_ctrl_c::init(void)
{
    ...
    // fixed floppy config, no SIM dependency
    const Bit8u fd0_devtype = BX_FDD_350HD;      // A: 3.5" 1.44M drive present
    const Bit8u fd0_type = BX_FLOPPY_NONE;       // no media inserted by default
    const unsigned fd0_status = BX_EJECTED;      // BX_INSERTED to auto-insert
    const bool fd0_readonly = false;
    const char* fd0_path = "none";               // set real image path if auto-insert

    const Bit8u fd1_devtype = BX_FDD_NONE;       // B: drive not present
    const Bit8u fd1_type = BX_FLOPPY_NONE;
    const unsigned fd1_status = BX_EJECTED;
    const bool fd1_readonly = false;
    const char* fd1_path = "none";

    const bool use_cmos_image = false;
    const unsigned bootdrive1 = BX_BOOT_DISKC;
    const unsigned bootdrive2 = BX_BOOT_FLOPPYA;
    const unsigned bootdrive3 = BX_BOOT_NONE;
    const bool floppy_sig_check = false;
    ...
    if (fd0_type != BX_FLOPPY_NONE) {
        if (fd0_status == BX_INSERTED) {
            BX_FD_THIS s.media[0].write_protected = fd0_readonly;
            if (evaluate_media(BX_FD_THIS s.device_type[0], fd0_type,
                fd0_path, &BX_FD_THIS s.media[0])) {
                BX_FD_THIS s.media_present[0] = 1;
#define MED (BX_FD_THIS s.media[0])
                if (MED.write_protected)
                    BX_FD_THIS s.media[0].write_protected = 1;
#undef MED
            }
            else {
    ...
    if (fd1_type != BX_FLOPPY_NONE) {
        if (fd1_status == BX_INSERTED) {
            BX_FD_THIS s.media[1].write_protected = fd1_readonly;
            if (evaluate_media(BX_FD_THIS s.device_type[1], fd1_type,
                fd1_path, &BX_FD_THIS s.media[1])) {
                BX_FD_THIS s.media_present[1] = 1;
#define MED (BX_FD_THIS s.media[1])
                if (MED.write_protected)
                    BX_FD_THIS s.media[1].write_protected = 1;
#undef MED
            }
            else {
    ...
    if (!use_cmos_image) {
        ...
        // Set the "extended" boot sequence, bytes 0x38 and 0x3D
        DEV_cmos_set_reg(0x3d, bootdrive1 | (bootdrive2 << 4));

        // Set the signature check flag in cmos
        DEV_cmos_set_reg(0x38, (floppy_sig_check ? 1 : 0) | (bootdrive3 << 4));
    }
    ...
    // runtime parameters disabled in this trimmed build
    BX_FD_THIS s.rt_conf_id = 0;
}
...
void bx_floppy_ctrl_c::write(Bit32u address, Bit32u value, unsigned io_len)
{
    ...
    case 0x3f4: /* diskette controller data rate select register */
        ...
        case 0: //BX_DEBUG(("  500 Kbps")); 
            break;
        case 1: //BX_DEBUG(("  300 Kbps")); 
            break;
        case 2: //BX_DEBUG(("  250 Kbps")); 
            break;
        case 3: //BX_DEBUG(("  1 Mbps")); 
            break;
        }
...
void bx_floppy_ctrl_c::floppy_command(void)
{
    ...
    case FD_CMD_OPTION:
        ...
        //X_DEBUG(("Option: ISO = %i", BX_FD_THIS s.command[1] & 1));
        enter_idle_phase();
        break;
...
bool bx_floppy_ctrl_c::set_media_status(unsigned drive, bool status)
{
    ...
    if (status == 0) {
        ...
        // runtime insert disabled without SIM-backed config
        return BX_FD_THIS s.media_present[drive] != 0;
    }
}
