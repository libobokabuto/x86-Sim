...
typedef int (*BxFetchDecode32Ptr)(const Bit8u* iptr, unsigned& remain, bxInstruction_c* i, unsigned b1, unsigned sse_prefix, const void* opcode_table);//74行
...
bxIAOpcodeTable BxOpcodesTable[] = {
    ...
    //&BX_CPU_C::BxError, &BX_CPU_C::BxError之前是d,e
...
extern Bit16u findOpcode(const Bit64u* opMap, Bit32u decmask);
...
int decoder32_nop(const Bit8u* iptr, unsigned& remain, bxInstruction_c* i, unsigned b1, unsigned sse_prefix, const void* opcode_table)
{
    ...
    if (sse_prefix == SSE_PREFIX_F3)
        return BX_IA_PAUSE;  //需要ia_dpcodes.def
    else
        return BX_IA_NOP;   //需要ia_dpcodes.def
}
...
if (b1 == 0x138 || b1 == 0x13a) {
    ...
    Bit8u opcode = *iptr++;
    if (b1 == 0x138) {
        opcode_table = BxOpcodeTable0F38[opcode];//需要ia_dpcodes.def
        b1 = 0x200 | opcode;
    }
    else if (b1 == 0x13a) {
        opcode_table = BxOpcodeTable0F3A[opcode];//需要ia_dpcodes.def
        b1 = 0x300 | opcode;
    }
    remain--;
}
