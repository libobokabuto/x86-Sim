...
struct bx_local_ata_channel_conf_t {
    bool enabled;
    Bit16u ioaddr1;
    Bit16u ioaddr2;
    Bit8u irq;
...
struct bx_local_ata_device_conf_t {
    int type;
    const char* model;
    const char* path;
    const char* mode;
...
void bx_hard_drive_c::init(void)
{
    ...
    for (channel = 0; channel < BX_MAX_ATA_CHANNEL; channel++) {
        const bx_local_ata_channel_conf_t& conf = bx_local_ata_channels[channel];

        if (conf.enabled) {
            BX_HD_THIS channels[channel].ioaddr1 = conf.ioaddr1;
            BX_HD_THIS channels[channel].ioaddr2 = conf.ioaddr2;
            BX_HD_THIS channels[channel].irq = conf.irq;
        }
        else {
    ...
    for (channel = 0; channel < BX_MAX_ATA_CHANNEL; channel++) {
        for (Bit8u device = 0; device < 2; device++) {
            ...
            const bx_local_ata_device_conf_t& devconf = bx_local_ata_devices[channel][device];
            ...
            if (devconf.type == BX_ATA_DEVICE_NONE) continue;
            ...
            if (devconf.type == BX_ATA_DEVICE_DISK) {
                ...
                int cyl = devconf.cylinders;
                int heads = devconf.heads;
                int spt = devconf.spt;
                int sect_size = devconf.sect_size;
                Bit64u disk_size = (Bit64u)cyl * heads * spt * sect_size;

                image_mode = devconf.mode;
                channels[channel].drives[device].hdimage = DEV_hdimage_init_image(image_mode,
                    disk_size, devconf.journal);
                ...
                if ((BX_HD_THIS channels[channel].drives[device].hdimage->open(devconf.path)) < 0) {
            ...
            else if (devconf.type == BX_ATA_DEVICE_CDROM) {
                ...
                BX_HD_THIS channels[channel].drives[device].cdrom.cd = DEV_hdimage_init_cdrom(devconf.journal);
                //BX_INFO(("CD on ata%d-%d: '%s'", channel, device, SIM->get_param_string("path", base)->getptr()));

                if (devconf.status == BX_INSERTED) {
            ...
            if (devconf.type != BX_ATA_DEVICE_NONE) {
            ...
            for (Bit8u device = 0; device < 2; device++) {
                ...
                if (devconf.type != BX_ATA_DEVICE_NONE) {
                    if (BX_DRIVE_IS_HD(channel, device)) {
                        ...
                        Bit8u translation = devconf.translation;
                        Bit8u bd = (devconf.biosdetect & 0x03);
...
void bx_hard_drive_c::write(Bit32u address, Bit32u value, unsigned io_len)
{
    ...
    case 0x07: // hard disk command 0x1f7
        ...
        case 0x22: //BX_ERROR(("write cmd 0x22 (READ LONG) not supported")); command_aborted(channel, 0x22); 
            break;
        case 0x23: //BX_ERROR(("write cmd 0x23 (READ LONG NO RETRY) not supported")); command_aborted(channel, 0x23); 
            break;
...
bool bx_hard_drive_c::set_cd_media_status(Bit32u handle, bool status)
{
    ...
    if (status == 0) {
        ...
        // insert cdrom
        if (BX_HD_THIS channels[channel].drives[device].cdrom.cd->insert_cdrom(devconf.path)) {
            //BX_INFO(("Media present in CD-ROM drive"));
            BX_HD_THIS channels[channel].drives[device].cdrom.ready = 1;
            Bit32u capacity = BX_HD_THIS channels[channel].drives[device].cdrom.cd->capacity();
            BX_HD_THIS channels[channel].drives[device].cdrom.max_lba = capacity - 1;
